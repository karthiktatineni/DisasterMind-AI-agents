...
Validation -> Planner -> Decision

## Purpose

The Planner Agent converts validated outputs into an operational emergency plan. It does not make the final executive decision; it organizes feasible actions into phases, dependencies, owners, contingencies, and escalation triggers.

## Responsibilities

* Convert validated recommendations into operational phases.
* Prioritize actions by urgency, risk, and dependency.
* Assign suggested operational owners or responsible teams.
* Define activation thresholds and escalation triggers.
* Preserve validation warnings and constraints.
* Prepare contingency branches for degraded infrastructure.

## Inputs

* Validation Agent report.
* Simulation Agent scenario outcomes.
* Evacuation Agent strategy.
* Logistics Agent deployment plan.
...
* The Planner must not ignore blocking validation failures.
* Each phase must include trigger, action, owner, and expected outcome.
* Contingencies must map to simulation risks.
* Plans must retain citations for evidence-sensitive decisions.

## Failure Handling

If validation has failed, the Planner produces a remediation plan focused on resolving blocking issues. It must not present the plan as execution-ready.
