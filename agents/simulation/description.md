...
The Simulation Agent stress-tests logistics and evacuation plans against disruption scenarios. It recalculates plan impacts when roads, hospitals, shelters, power, communications, or hazard conditions degrade.

## Responsibilities

* Run configured disaster simulations.
* Test road failure, hospital failure, shelter saturation, flood expansion, cyclone escalation, earthquake aftershock, power grid failure, and communication failure.
* Calculate plan deltas and operational consequences.
* Recommend mitigations for each failed assumption.
* Produce machine-readable simulation reports.

## Inputs

* Evacuation Agent strategy.
* Logistics Agent resource plan.
* Prediction Agent impact estimates.
* Risk Agent critical zones.
...
* Simulation engine.
* Capacity recalculation services.
* Infrastructure failure models.
* Scenario result comparator.

## Validation Rules

* Every simulation must identify the changed assumption.
* Scenario deltas must reference affected plan sections.
* Mitigations must be tied to specific resilience gaps.
* Failed simulations must not be hidden from downstream agents.

## Failure Handling

If a simulation cannot run, the agent returns the failed scenario name and reason. The Validation Agent decides whether the missing simulation blocks final planning.
