...
User -> Coordinator -> Intelligence

## Purpose

The Coordinator Agent is the chief orchestration layer for DisasterMind AI. It receives the user scenario, converts it into an executable disaster-response workflow, invokes each specialized agent in sequence, and preserves request-level traceability across the complete chain.

## Responsibilities

* Interpret the disaster scenario, location, severity indicators, timeline, and operational constraints.
* Create a deterministic execution plan for all downstream agents.
* Enforce the canonical sequence: Intelligence, Risk, Prediction, Logistics, Evacuation, Simulation, Validation, Planner, Decision, Reporting.
* Pass only structured JSON outputs between agents.
* Track agent status, retry failures, and record fallback decisions.
* Maintain the shared `request_id` and audit metadata for the whole run.
* Ensure final recommendations are generated only after validation has passed or produced explicit warnings.

## Inputs

* User scenario text.
* Disaster type and affected geography.
* Time horizon, such as current emergency, 24-hour plan, or 72-hour plan.
* Available external data-source configuration.
...
* NVIDIA Build / NIM generation API through the shared backend generation client.
* Redis for short-term workflow state.
* PostgreSQL for persistent audit records.
* Structured logging and OpenTelemetry tracing.

## Validation Rules

* The execution plan must include all mandatory agents.
* The Validation Agent must run before Planner and Decision.
* The Reporting Agent must run after the Decision Agent.
* Missing critical scenario fields must be marked as data gaps, not invented.

## Failure Handling

If a downstream agent fails, the Coordinator retries once with the same structured input and records the failure. If the retry fails, it produces a fallback task result with `status: "failed"` and sends it to the Validation Agent for explicit handling.
