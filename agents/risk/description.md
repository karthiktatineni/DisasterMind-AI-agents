...
Intelligence -> Risk -> Prediction

## Purpose

The Risk Agent estimates disaster severity, population exposure, critical zones, and immediate operational risk. It turns grounded context and scenario indicators into a structured risk profile that downstream agents can calculate against.

## Responsibilities

* Assess hazard severity using weather, geography, population, and infrastructure inputs.
* Estimate affected population and vulnerable groups.
* Identify critical zones requiring priority attention.
* Explain the main risk drivers.
* Flag missing or low-confidence risk inputs.

## Inputs

* Intelligence Agent context and citations.
* Weather indicators.
* Population and density information.
...
* Geospatial risk scoring service.
* Population exposure calculator.
* Infrastructure vulnerability rules.
* NVIDIA Build / NIM generation API for structured reasoning summaries.

## Validation Rules

* Risk score must be between 0 and 1.
* Affected population must not exceed the known population unless marked as uncertain.
* Critical zones must include supporting risk drivers.
* Risk level must correspond to the numeric risk score.

## Failure Handling

If a required input is missing, the agent returns a partial risk profile with confidence reduced and `status: "partial"`. It must include exact missing fields in `next_actions`.
