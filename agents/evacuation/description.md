...
The Evacuation Agent creates the movement strategy for exposed populations. It assigns evacuation zones, routes, shelters, transport needs, and phased movement priorities based on risk, capacity, and logistics constraints.

## Responsibilities

* Identify evacuation priority zones.
* Generate primary and backup evacuation routes.
* Assign shelters based on capacity and distance.
* Estimate transport needs for vulnerable groups.
* Incorporate Logistics Agent constraints.
* Flag route, shelter, or timing conflicts.

## Inputs

* Logistics Agent output.
* Risk Agent critical zones.
* Prediction Agent evacuation need.
* Road network data.
...
* Shelter assignment optimizer.
* Population movement estimator.
* Map and geospatial data sources.

## Validation Rules

* Shelter assignments must not exceed known capacity.
* Evacuation routes must avoid known blocked or unsafe roads.
* Vulnerable populations must be included in transport requirements.
* Zone priority must align with Risk Agent critical zones.

## Failure Handling

If route data is unavailable, the agent returns evacuation requirements and shelter capacity analysis without claiming route feasibility. Validation must mark route feasibility as blocked or uncertain.
