...
Prediction -> Logistics -> Evacuation

## Purpose

The Logistics Agent calculates emergency resources, deployment capacity, resource gaps, and staging requirements. It transforms predicted impact into an actionable resource deployment plan.

## Responsibilities

* Calculate required ambulances, rescue teams, medical supplies, food, water, and shelter support.
* Match resource needs against available inventory.
* Identify shortfalls and priority procurement needs.
* Recommend staging locations and deployment waves.
* Preserve operational constraints for Evacuation and Planner agents.
...
* Hospital capacity.
* Shelter capacity.
* Available vehicles, rescue teams, supplies, and personnel.
* Road and staging constraints when available.
...
* Resource calculator.
* Inventory service.
* Hospital and shelter capacity datasets.
* Constraint checker.

## Validation Rules

* Resource totals must be numeric and non-negative.
* Deployment waves must not allocate more resources than available.
* Known shortfalls must be explicit.
* Critical medical resource gaps must be marked as high priority.

## Failure Handling

If inventory data is unavailable, the agent returns an estimated resource plan with low confidence and clear assumptions. The Validation Agent must decide whether the plan is feasible.
