...
Risk -> Prediction -> Logistics

## Purpose

The Prediction Agent runs the ML prediction layer for expected damage, casualties, risk amplification, and evacuation demand. It supplies quantitative forecasts for Logistics and Evacuation planning.

## Responsibilities

* Run the configured predictive model.
* Estimate damage score, casualty estimate, risk score, and evacuation need.
* Use features such as wind speed, rainfall, population density, elevation, historical damage, temperature, and humidity.
* Return prediction confidence and model metadata.
* Report feature gaps and out-of-distribution inputs.

## Inputs

* Risk Agent output.
* Weather and environment features.
* Historical disaster damage references.
* Population density and elevation.
* Current model registry configuration.
...
* LightGBM fallback model when configured.
* Scikit-Learn preprocessing pipeline.
* Model registry and version metadata.

## Validation Rules

* Numeric scores must stay within expected ranges.
* The model version must be recorded.
* Feature gaps must lower confidence.
* Evacuation need must be compatible with affected population from the Risk Agent.

## Failure Handling

If the model cannot run, the agent returns `status: "failed"` with the failing stage, model name, and missing features. The Coordinator may retry or pass the failure to Validation for blocking assessment.
