...
Simulation -> Validation -> Planner

## Purpose

The Validation Agent verifies that all upstream outputs are consistent, feasible, evidence-grounded, and safe to use for operational planning. It is mandatory and must run before Planner and Decision.
...
* Confirm evidence and citation coverage.
...
* Risk Agent risk profile.
* Prediction Agent forecast.
* Logistics Agent resource deployment plan.
* Evacuation Agent strategy.
* Simulation Agent scenario results.
...
* Logistics math verifier.

## Validation Rules

* Impossible capacity plans must fail validation.
* Missing evidence for major recommendations must create a warning or blocking issue.
* Prediction and evacuation numbers must be internally consistent.
* Validation must produce explicit reasons for each warning or failure.

## Failure Handling

If validation fails, downstream agents must receive `validation_status: "failed"` and the Planner must build only a remediation plan unless the Coordinator explicitly stops execution.
