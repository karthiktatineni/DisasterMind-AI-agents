...
Decision -> Reporting -> Final Output

## Purpose

The Reporting Agent turns the final decision package into exportable, user-facing, and audit-ready reports. It is responsible for the final presentation layer, not for changing the decision logic.

## Responsibilities

* Generate the Executive Summary.
* Generate the Emergency Action Plan.
* Generate the Resource Deployment Report.
* Generate the Evacuation Report.
* Generate the Simulation Report.
* Generate the Decision Trace.
* Generate the Audit Trail.
* Export reports as Markdown, JSON, and PDF when export services are available.
...
* Evidence citations and audit metadata.
...
* Reports must preserve final Decision Agent recommendations.
* Validation warnings and unresolved risks must be visible.
* Citations must remain attached to evidence-based recommendations.
* Export failures must not alter the report content.

## Failure Handling

If a PDF or file export fails, the Reporting Agent still returns Markdown and JSON report content with `status: "partial"` and lists failed export formats in `next_actions`.
