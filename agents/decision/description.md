...
Planner -> Decision -> Reporting

## Purpose

The Decision Agent creates the final recommendation by evaluating the validated operational plan, resolving conflicts, ranking actions, and producing an auditable decision trace.

## Responsibilities

* Compare all upstream agent outputs.
* Resolve conflicts using validation results and evidence strength.
* Prioritize final actions.
* Produce the final emergency strategy.
* Generate a confidence score and explanation chain.
* Mark unresolved risks for the Reporting Agent.

## Inputs

* Planner Agent operational plan.
* Validation Agent report.
* Simulation Agent results.
* Logistics and Evacuation plans.
...
* NVIDIA Build / NIM generation API for structured decision synthesis.

## Validation Rules

* Final actions must be traceable to upstream evidence or calculations.
* Blocking validation failures must prevent an execution-ready decision.
* Confidence must reflect evidence quality, model confidence, simulation coverage, and validation status.
* The decision trace must include rejected alternatives when relevant.

## Failure Handling

If the Decision Agent cannot resolve contradictions, it returns `status: "needs_human_review"` and lists the conflicts. The Reporting Agent must present the unresolved issues prominently.
