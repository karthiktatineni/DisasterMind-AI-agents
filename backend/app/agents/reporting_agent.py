...
class ReportingAgent(AgentRunner):
    ...
    async def run(self, ctx: AgentContext, report_text: str = "", provider: str = "") -> AgentResult:
        async def _run_impl(_ctx: AgentContext) -> AgentResult:
            decision = _ctx.outputs.get("decision", {})
            validation = _ctx.outputs.get("validation", {})
            planner = _ctx.outputs.get("planner", {})
            prediction = _ctx.outputs.get("prediction", {})
            retrieved = _ctx.rag.get("retrieved_disasters", [])
            ...
            sections = {
                ...
                "simulation_findings": _ctx.outputs.get("simulation", {}).get("scenario_results", []),
        ...
        return await self._timed_run_async(ctx, _run_impl)
